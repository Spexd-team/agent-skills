...
| [`spexd-implementing`](skills/spexd-implementing/SKILL.md) | **Executing the spec.** Pull a task and its full ancestor chain (task → design → acceptance criteria → requirement → feature) as context, build to that spec without changing it, and reflect progress back onto the task's lifecycle status. Work always lands one task at a time: given a parent entity, expand it into its tasks, order them, and stack a branch and a PR per task. Use it to pick up, build, or ship work that's tracked in Spexd. |
